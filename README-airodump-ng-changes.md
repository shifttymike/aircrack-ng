...
```sh
aircrack-ng -w wordlist.txt wpa-handshakes-0727-153045.wpa
hashcat -m 22000 -a 0 wpa-handshakes-0727-153045.22000 wordlist.txt
```
...
- `v` opens a channel availability overlay for the active band; green entries are confirmed usable by the hopper, red entries are unavailable from the driver/regdom list or were refused during this run, white entries haven't been attempted yet.
...
- Runs `deauth -0 1 -a <AP_MAC> -c <STA_MAC> <wlan_if>` against all observed stations associated with the selected AP.
...
- Leaves the capture parked on the selected AP channel after completion (to capture handshakes).

## Band switching

- `b` cycles the active band preset forward: 2.4 GHz -> 5 GHz -> 6 GHz -> 2.4 GHz.
- `B` cycles the active band preset backward.
- Band switching skips bands that are not supported by the current adapter.
